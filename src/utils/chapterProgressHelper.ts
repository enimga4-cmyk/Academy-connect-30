...
export function getChapterProgressRecord(
  ...
  const keyWithSubject = `${subject}_${noteId}`;
  return chapterProgressMap[keyWithSubject] || chapterProgressMap[noteId] || null;
}
...
export function calculateSubjectSummary(
  ...
  const stageCounts: Record<string, number> = {
    "Not Started": 0,
    "Reading": 0,
    "Completed First Reading": 0,
    "First Revision Completed": 0,
    "Second Revision Completed": 0,
    "Third Revision Completed": 0,
    "PYQs Solved": 0,
    "Fully Prepared": 0
  };
  ...
  notes.forEach((note) => {
    const record = getChapterProgressRecord(note.id, subject, chapterProgressMap);
    const statusLabel = normalizeStatusLabel(record?.selectedStatus);
    const config = getStatusConfig(statusLabel);
    const pct = config.percent;
    sumPercent += pct;

    if (stageCounts[config.label] !== undefined) {
      stageCounts[config.label]++;
    } else {
      stageCounts[config.label] = (stageCounts[config.label] || 0) + 1;
    }

    if (config.label === "Not Started") {
      notStarted++;
    } else if (config.label === "Reading") {
      reading++;
    }
  ...
  const overallProgressPercent = Math.round(sumPercent / totalChapters);
