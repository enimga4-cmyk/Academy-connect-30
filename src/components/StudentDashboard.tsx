...
export function StudentMyTab({ 
  ...
  const handleSaveChapterProgress = async (status: string, remarks: string) => {
    ...
    const key = `${selectedSubject}_${progressModalNote.id}`;
    ...
    <div className="flex flex-col gap-4 animate-fadeIn" id="student-my-tab">
              ...
              <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-3 scrollbar-thin" id="study-right-notes">
                {selectedNotes.length === 0 ? (
                  ...
                  selectedNotes.map((note) => {
                    const progRecord = getChapterProgressRecord(note.id, selectedSubject, student.chapterProgress);
                    const statusConfig = getStatusConfig(progRecord?.selectedStatus);
                    const calculatedPercent = progRecord?.calculatedProgress ?? statusConfig.percent;
                    ...
                    return (
                      <div key={note.id} className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-950/20 p-3.5 hover:border-slate-200 dark:hover:border-slate-750 hover:bg-slate-50/80 dark:hover:bg-slate-950/30 transition-all flex flex-col gap-3">
                        ...
                        {/* Chapter Card Display: Status Chip, Progress Bar, First Line Remark */}
                        <div className="flex flex-col gap-1.5 pt-2 border-t border-slate-100 dark:border-slate-800/80">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-1.5">
                              <span className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500">Status:</span>
                              <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border text-[10px] font-extrabold ${statusConfig.badgeClass}`}>
      ...
      {progressModalNote && selectedSubject && (
        <ChapterProgressBottomSheet
          ...
          initialProgress={getChapterProgressRecord(progressModalNote.id, selectedSubject, student.chapterProgress)}
